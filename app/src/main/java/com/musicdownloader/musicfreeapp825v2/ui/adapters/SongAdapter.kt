...
import com.musicdownloader.musicfreeapp825v2.ui.SongPickerActivity
...
class SongAdapter(
    ...
    // Overrides the default (first-launch) sort for this list without touching [rawOrderExposed]
    // (which stays the reader's raw/native order). The main Songs library tab passes
    // ByAddDateDescending here so it defaults to newest-first.
    defaultSortTypeOverride: Sorter.Type? = null
    ...
    initialSortType =
        (if (helper != null) Sorter.Type.NaturalOrder else (if (folder) Sorter.Type
            .ByFilePathAscending else defaultSortTypeOverride ?: rawOrderExposed ?: Sorter.Type.ByTitleAscending)),
    canSort = true,
