...
class OnlineSearchViewModel : ViewModel() {
    ...
    // Recent search queries (most-recent-first), persisted via SearchHistoryManager. Shown as
    // tappable suggestions while the query box is empty. Not in the sample — Gramophone-only.
    ...
    fun loadHistory(context: Context) {
        _history.value = SearchHistoryManager.get(context)
    }

    fun addToHistory(context: Context, query: String) {
        _history.value = SearchHistoryManager.add(context, query)
    }

    fun removeFromHistory(context: Context, query: String) {
        _history.value = SearchHistoryManager.remove(context, query)
    }
